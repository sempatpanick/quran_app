...
class CustomItemSurah extends StatelessWidget {
  ...
  const CustomItemSurah({Key? key, required this.dataSurah, required this.isFavorite}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Slidable(
      ...
      endActionPane: ActionPane(
        ...
        children: [
          isFavorite
           ? SlidableAction(
              onPressed: (contexts) async {
                final surahViewModel = Provider.of<SurahViewModel>(context, listen: false);
                final favorite = await surahViewModel.removeFavorite(dataSurah.number);
                ...
                edgeAlert(
                    ...
                    gravity: Gravity.top,
                    backgroundColor: favorite.status ? Colors.green : bgColorRedLight
                );
              },
          ...
          : SlidableAction(
            onPressed: (contexts) async {
              final surahViewModel = Provider.of<SurahViewModel>(context, listen: false);
              final favorite = await surahViewModel.addToFavorite(dataSurah.number);
      ...
      child: ListTile(
        onTap: () {
          Navigator.pushNamed(context, DetailSurahScreen.routeName, arguments: {'number': dataSurah.number, 'title': dataSurah.name.transliteration.id});
        },
        title: Text(
